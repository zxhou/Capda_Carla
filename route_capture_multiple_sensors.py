...
import carla
from Import.agents.navigation.behavior_agent import BehaviorAgent
...
def parser():
    ...
    argparser.add_argument(
        '--route',
        help='Set the route file (default: ./Route_Town07.txt)',
        default='./Route_Town07.txt',
        type=str)
    argparser.add_argument(
        '--fix_route',
...
def set_start_end_pos(world, args, mode = 'random'):
    if mode == 'random':
        ...
        spawn_point = random.choice(all_optional_position)
        random.shuffle(all_optional_position)
        if all_optional_position[0] != spawn_point:
        ...
        destination = carla.Location(line_split[6], line_split[7], line_split[8])
...
def main():
    ...
    try:
        ...
        agent.set_destination(vehicle_destination, agent._vehicle.get_location())
